...
def read(channel)
  PiPiper::Spi.begin do |spi|
    # ch1の場合は2進数では [00000001, 10010000, 00000000] の配列を送る
    adc = spi.write [0b0001, (0b1000 + channel) << 4, 0b0000]
    ((adc[1] & 0b0011) << 8) + adc[2] # &演算子はビット積
    # 上記は次のような処理をしている
...
コードの `adc[1] & 0b0011` は1bit目〜0bit目をビットマスクしています。それを、8bit分を前にビットシフトしています。
...
1. サンプルソースではわかりやすくするために `0b0011` などと2進数をしました。変更しやすいように、16進数に置き換えてみましょう。例えば、`0b0011` は `0x3` になります。
